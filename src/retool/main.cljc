(ns retool.main
  "Kotodama WASM entrypoint for the Retool clean-room actor (L4) — Clojure port.
  ...
  facts (`retool.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "retool")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"retool.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "retool-compat" :tier tier :entities entities} 200])
...
(defn start! [] :retool-compat/ready)
